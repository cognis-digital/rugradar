...
[![PyPI](https://img.shields.io/pypi/v/cognis-rugradar.svg)](https://pypi.org/project/cognis-rugradar/)
[![CI](https://github.com/cognis-digital/rugradar/actions/workflows/ci.yml/badge.svg)](https://github.com/cognis-digital/rugradar/actions)
...
RUGRADAR exists for one job — token contract risk scanner detecting honeypots, hidden mint/blacklist functions, owner backdoors, and unlocked liquidity before you ape. — and does it without a SaaS bill or heavyweight setup.
...
```bash
rugradar --version
rugradar scan .                      # scan the current project
rugradar scan . --format json
rugradar scan . --fail-on high       # non-zero exit for CI gates
rugradar mcp                         # expose as an MCP server (Cognis.Studio / Claude Desktop / Cursor)
```
...
Built in the spirit of **Token Sniffer / GoPlus Security API**, re-framed for the Cognis approach: single-purpose, self-hostable,
MCP-native, and unified with the rest of the Suite. Missing a credit? Open a PR.

## How it fits the Cognis Neural Suite

`rugradar` is one of the **100+ tools** in the [Cognis Neural Suite](https://github.com/cognis-digital).
