...
`rugradar` scans a token contract (Solidity source or ABI JSON) for honeypots, hidden mint/blacklist, owner backdoors, and fee traps. Exit codes: `0` below the `--fail-on` verdict, `2` verdict met/exceeded (CI gate), `1` usage/IO error.

1. **Install**
   ```bash
   pip install rugradar
   ```

2. **Scan a Solidity source file:**
   ```bash
   rugradar scan Token.sol
   ```

3. **Scan an ABI**, or pipe from stdin — input kind auto-detects (override with `--kind {auto,solidity,abi}`):
   ```bash
   rugradar scan Token.abi.json --kind abi
   cat Token.sol | rugradar scan -
   ```

4. **Read JSON output** (verdict, risk score, findings) for tooling:
   ```bash
   rugradar scan Token.sol --format json | jq '.verdict, .risk_score'
   ```

5. **Use in CI** — gate on the risk verdict with `--fail-on {never,caution,high_risk,critical,any}`:
   ```bash
   rugradar scan Token.sol --fail-on high_risk || echo "contract flagged"
   ```
