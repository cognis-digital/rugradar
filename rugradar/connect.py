"""Native cognis-connect emit for rugradar — forward findings to any platform.

Maps rugradar's JSON output to the canonical `Finding` and forwards it via
...
Usage:
    rugradar ... --format json | rugradar-emit --to stix
    rugradar-emit --to slack --url $WEBHOOK --dry-run < findings.json
"""
...
def map_record(rec: dict) -> dict:
    """Tailor a rugradar record toward the Finding contract before normalize().
